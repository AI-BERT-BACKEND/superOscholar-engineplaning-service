...
public class AdjustEstimationsUseCaseImpl implements AdjustEstimationsUseCase {
    ...
    public AdjustEstimationsResponse adjustEstimations(String studentId, AdjustEstimationsRequest request) {
        ...
        if (request.getActualTime() == null || request.getActualTime() <= 0) {
            ...
            return buildSkippedResponse("No se registró tiempo real. El factor de ajuste no fue actualizado.");
        }

        // 1. Record this task's ratio
        if (request.getEstimatedDurationMinutes() != null && request.getEstimatedDurationMinutes() > 0) {
            double ratio = (double) request.getActualTime() / request.getEstimatedDurationMinutes();
        ...
        if (ratios.size() < MIN_SAMPLES) {
            ...
            return buildSkippedResponse(
                    "Aún no hay suficientes datos para ajustar estimaciones. "
                            + "Se necesitan al menos " + MIN_SAMPLES + " tareas completadas del mismo tipo.");
        }
