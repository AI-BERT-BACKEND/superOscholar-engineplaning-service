 ...
 *
 * <p>Crosses each task's {@code estimatedDurationMinutes} with the student's
 ...
 * </ul>
 * Tasks with {@code academicWeight > 0.30} are prioritized in the report (RN-02).
 */
@Service
@Slf4j
...
public class DetectHighRiskTasksUseCaseImpl implements DetectHighRiskTasksUseCase {
    ...
    public HighRiskTaskResult detectHighRiskTasks(String studentId) {
        ...
        if (activeTasks.isEmpty()) {
            log.info("AIB-22.3: No active tasks for student '{}'. Skipping risk detection.", studentId);
        ...
        if (schedules == null || schedules.isEmpty()) {
            log.info("AIB-22.3 FA-01: No availability configured for student '{}'.", studentId);
        ...
        log.info("AIB-22.3: Detected {} at-risk tasks for student '{}'", totalAtRisk, studentId);
    ...
    private RiskTaskDetail evaluateTask(PlanningTask task, Map<LocalDate, Integer> availabilityMap, String studentId) {
    ...
    private RiskLevel determineRiskLevel(int availableMinutes, int estimatedMinutes) {
        if (estimatedMinutes <= 0) return RiskLevel.NONE;
        double ratio = (double) availableMinutes / estimatedMinutes;
        if (ratio < HIGH_RISK_THRESHOLD) return RiskLevel.HIGH;
        if (ratio < MEDIUM_RISK_THRESHOLD) return RiskLevel.MEDIUM;
